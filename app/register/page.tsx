...
export default function RegisterPage() {
  ...
  async function handleRegister(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (data?.requireEmailVerification) {
        ...
        setInfoMessage("Te enviamos un codigo por email. Ingresalo abajo para activar tu cuenta.");
        return;
      }
      ...
      const message = error instanceof Error ? error.message : "Error inesperado al registrar usuario.";
  ...
  async function handleVerifyEmail(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Error inesperado al verificar email.";
      setErrorMessage(message);
    } finally {
      setIsSubmitting(false);
    }
  ...
  async function handleResendCode() {
    ...
    try {
      ...
      if (data?.success) {
        setInfoMessage("Codigo reenviado. Revisa tu bandeja de entrada.");
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : "No se pudo reenviar el codigo.";
      setErrorMessage(message);
    }
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-black px-4 py-8">
      <main className="w-full max-w-md rounded-2xl border border-zinc-800 bg-gradient-to-b from-zinc-900 to-zinc-950 p-6 shadow-[0_20px_80px_rgba(0,0,0,0.45)] sm:p-7">
        <h1 className="text-center text-3xl font-semibold tracking-tight text-white">
          {needsVerification ? "Verifica tu email" : "Crea tu cuenta en StudyFlow AI"}
        </h1>
        <p className="mt-2 text-center text-base text-zinc-400">
          {needsVerification ? "Ingresa el codigo que enviamos a tu correo" : "Empieza a estudiar con tu espacio personal"}
        </p>
        ...
        {!needsVerification ? (
          <>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              {infoMessage ? (
                <p className="rounded-lg border border-teal-400/30 bg-teal-500/10 px-3 py-2 text-sm text-teal-200">
                  {infoMessage}
                </p>
              ) : null}
          ...
          <form onSubmit={handleVerifyEmail} className="mt-6 space-y-4">
            <div>
              <label htmlFor="otp" className="mb-2 block text-base font-medium text-zinc-100">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-zinc-700 bg-zinc-950/80 px-4 text-base tracking-[0.2em] text-zinc-100 outline-none ring-zinc-500 transition focus:ring-2"
                placeholder="123456"
              />
            </div>
            ...
            <button
              ...
              className="inline-flex h-11 w-full items-center justify-center rounded-xl bg-zinc-100 text-base font-semibold text-zinc-900 transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex h-11 w-full items-center justify-center rounded-xl border border-zinc-700 text-sm font-semibold text-zinc-100 transition hover:border-zinc-500 hover:bg-zinc-800 cursor-pointer"
            >
              Reenviar codigo
            </button>
          </form>
        ...
        <p className="mt-6 text-center text-base text-zinc-400">
          Ya tienes cuenta?{" "}
          <Link href="/login" className="font-semibold text-zinc-100 hover:text-white">
            Inicia sesion
          </Link>
        </p>
      </main>
